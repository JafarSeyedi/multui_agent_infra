...
#### 1.1 Resource Registry (`resource/resource_registry.py`)
...
#### 1.2 Work Distribution Engine (`resource/work_distributor.py`)
...
#### 1.3 Work Queue (`resource/work_queue.py`)
...
#### 1.4 Unified Cartable (`resource/cartable.py`)
...
#### 2.1 Workflow Context Entity (`context/workflow_context.py`)
...
#### 2.2 RDBMS Context Store (`context/rdbms_context_store.py`)
...
#### 2.3 BPMN 2 RDBMS Extension (`context/bpmn_rdbms_extension.py`)
...
#### 2.4 Context Sharing API (`context/context_sharing.py`)
...
#### 3.1 Role Model (`auth/role_model.py`)
...
#### 3.2 Context-Sensitive RBAC (`auth/context_rbac.py`)
...
#### 3.3 Authorization Service (`auth/auth_service.py`)
...
#### 3.4 Task-Level Authorization (`auth/task_auth.py`)
...
#### 3.5 Admin Authorization (`auth/admin_auth.py`)
...
#### 4.1 Form Model (`forms/form_model.py`)
...
#### 4.2 Entity-Driven Form Generator (`forms/entity_form_generator.py`)
...
#### 4.3 Workflow Form Types (`forms/workflow_forms.py`)
...
#### 4.4 Form Layout Engine (`forms/layout_engine.py`)
...
#### 4.5 Form Submission & Validation (`forms/form_submission.py`)
...
### Affected Files
- New: `forms/form_model.py`, `entity_form_generator.py`, `workflow_forms.py`, `layout_engine.py`, `form_submission.py`
...
#### 5.1 Service Registry (`service/service_registry.py`)
...
#### 5.2 Workflow Service Exposure (`service/workflow_exposure.py`)
...
#### 5.3 Message-Based Service Invocation (`service/message_service.py`)
...
#### 5.4 Choreography Service Exposure (`service/choreography_service.py`)
...
#### 5.5 Global Task Service (`service/global_task_service.py`)
...
#### 6.1 Service Consumer Framework (`service/service_consumer.py`)
...
#### 6.2 Service Task Binding (`service/service_task_binding.py`)
...
#### 6.3 Connector Framework (`service/connector_framework.py`)
...
#### 6.4 Service Orchestration Patterns (`service/orchestration_patterns.py`)
...
### Affected Files
- New: `service/service_consumer.py`, `service_task_binding.py`, `connector_framework.py`, `orchestration_patterns.py`
...
#### 7.1 Distributed Event Bus (`bus/distributed_event_bus.py`)
...
#### 7.2 Message Bus Integration (`bus/message_bus_bridge.py`)
...
#### 7.3 Scalability Patterns (`bus/scalability.py`)
...
#### 7.4 Distributed State Management (`bus/distributed_state.py`)
...
#### 7.5 Event Sourcing & CQRS (`bus/event_sourcing.py`)
...
#### 8.1 Tool-Call Task Type (`bpmn/tool_call_task.py`)
...
#### 8.2 Tool Registry (`bpmn/tool_registry.py`)
...
#### 8.3 Tool Execution Framework (`bpmn/tool_execution.py`)
...
#### 8.4 BPMN Extension Elements (`bpmn/tool_bpmn_extension.py`)
...
### Affected Files
- New: `bpmn/tool_call_task.py`, `tool_registry.py`, `tool_execution.py`, `tool_bpmn_extension.py`
...
#### 9.1 Agent Task Types (`bpmn/agent_tasks.py`)
- New BPMN task types:
...
#### 9.2 Agent Registry (`bpmn/agent_registry.py`)
...
#### 9.3 Agent Execution Framework (`bpmn/agent_execution.py`)
...
#### 9.4 Agent-Process Integration (`bpmn/agent_process_integration.py`)
...
#### 9.5 BPMN Extension Elements (`bpmn/agent_bpmn_extension.py`)
...
### Affected Files
- New: `bpmn/agent_tasks.py`, `agent_registry.py`, `agent_execution.py`, `agent_process_integration.py`, `agent_bpmn_extension.py`
...
#### 10.1 Task Metadata Model (`task/task_metadata.py`)
...
#### 10.2 Task Notes API (`task/task_notes.py`)
...
#### 10.3 Task Progress API (`task/task_progress.py`)
...
#### 10.4 File Attachment Framework (`task/task_attachments.py`)
...
#### 10.5 User Task Enhancement (`task/user_task_enhancement.py`)
...
#### 11.1 Attachment-Document Bridge (`attachment/attachment_document_bridge.py`)
...
#### 11.2 Workflow Attachment Context (`attachment/workflow_attachment_context.py`)
...
#### 11.3 RAG Integration (`attachment/rag_integration.py`)
...
#### 11.4 Document Ingestion Pipeline (`attachment/document_ingestion.py`)
...
### Affected Files
- New: `attachment/` package
- Modify: `task/task_attachments.py` (document integration)
...
#### 12.1 Unified Storage Model (`storage/unified_storage.py`)
...
#### 12.2 Token Storage (`storage/token_storage.py`)
...
#### 12.3 Instance Storage (`storage/instance_storage.py`)
...
#### 12.4 Context & Data Object Storage (`storage/context_storage.py`)
...
#### 12.5 Time-Series Optimization (`storage/timeseries_optimization.py`)
...
#### 13.1 MCP Server Core (`mcp/mcp_server.py`)
...
#### 13.2 MCP Tools — Orchestration Definitions (`mcp/tools/definition_tools.py`)
...
#### 13.3 MCP Tools — Orchestration Runtime (`mcp/tools/runtime_tools.py`)
...
#### 13.4 MCP Tools — Logs & Monitoring (`mcp/tools/log_tools.py`)
...
#### 13.5 MCP Resources (`mcp/resources/orchestration_resources.py`)
...
#### 13.6 MCP Prompts (`mcp/prompts/orchestration_prompts.py`)
...
#### 13.7 Developer Tools Integration (`mcp/tools/dev_tools.py`)
...
### Affected Files
- New: `mcp/` package (server, tools, resources, prompts)
...
#### 14.1 Template Engine (`template/template_engine.py`)
...
#### 14.2 Workflow Control Patterns (`template/patterns/control_patterns.py`)
...
#### 14.3 Workflow Resource Patterns (`template/patterns/resource_patterns.py`)
...
#### 14.4 Workflow Data Patterns (`template/patterns/data_patterns.py`)
...
#### 14.5 Workflow Business Rules Patterns (`template/patterns/business_rules_patterns.py`)
...
#### 14.6 Pattern Library (`template/patterns/pattern_library.py`)
...
#### 14.7 Schema Validation (`template/schema/schema_validator.py`)
...
### Affected Files
- New: `template/` package (engine, patterns, schema, library)
...
#### 15.1 Best Practices Knowledge Base (`knowledge/best_practices_kb.py`)
...
#### 15.2 Practice Import & Transformation (`knowledge/practice_importer.py`)
...
#### 15.3 Context-Aware Recommendation Engine (`knowledge/recommendation_engine.py`)
...
#### 15.4 Domain Knowledge Graph (`knowledge/domain_knowledge_graph.py`)
...
#### 15.5 Compliance & Governance Rules (`knowledge/compliance_rules.py`)
...
#### 15.6 Infrastructural vs Business-Specific Separation (`knowledge/knowledge_taxonomy.py`)
...
### Affected Files
- New: `knowledge/` package (kb, importer, recommendations, graph, compliance, taxonomy)
...
- Modify: `template/pattern_library.py` (link patterns to practices)
