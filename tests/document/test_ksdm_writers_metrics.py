# tests/document/test_ksdm_writers_metrics.py
"""
Tests for KSDM metrics writers.
"""
import json
...
@pytest.mark.asyncio
async def test_metrics_json_writer(sample_metrics_doc):
    writer = KSDMMetricsJSONWriter()
    data = await writer.write(sample_metrics_doc)
...
@pytest.mark.asyncio
async def test_metrics_yaml_writer(sample_metrics_doc):
    writer = KSDMMetricsYAMLWriter()
    data = await writer.write(sample_metrics_doc)
...
@pytest.mark.asyncio
async def test_metrics_csv_writer(sample_metrics_doc):
...
@pytest.mark.asyncio
async def test_metrics_roundtrip(sample_metrics_doc):
    json_writer = KSDMMetricsJSONWriter()
    json_parser = KSDMMetricsJSONParser()

    data = await json_writer.write(sample_metrics_doc)
