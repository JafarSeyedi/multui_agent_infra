# tests/document/test_ksdm_parsers_metrics.py
"""
Tests for KSDM metrics parsers.
"""
import json
...
@pytest.mark.asyncio
async def test_metrics_json_parser(sample_metrics_json):
    parser = KSDMMetricsJSONParser()
    data = json.dumps(sample_metrics_json).encode("utf-8")
    doc = await parser.parse_bytes(data, "test-metrics", "test.isfm.json")
    assert doc.document_id == "test-metrics"
...
@pytest.mark.asyncio
async def test_metrics_yaml_parser(sample_metrics_json):
    parser = KSDMMetricsYAMLParser()
    data = yaml.dump(sample_metrics_json).encode("utf-8")
    doc = await parser.parse_bytes(data, "test-metrics-yaml", "test.isfm.yaml")
    assert doc.kind == DocumentStandard.KSDM
    assert doc.granularity.value == "day"
    assert len(doc.metrics) == 1
...
def test_parser_extensions():
    assert ".isfm.json" in KSDMMetricsJSONParser().supported_extensions
