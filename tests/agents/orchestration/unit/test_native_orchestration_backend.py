...
from agents.orchestration.native_backend import NativeOrchestrationBackend
from agents.orchestration.autogen_backend import AutoGenOrchestrationBackend  # for Shared helpers if needed
...
@pytest.mark.asyncio
async def test_sequential_strategy_updates_context_and_returns_success():
    ...
    request = make_request(scenario="sequential", tasks=tasks, shared_context={"foo": "bar"})
...
async def test_round_robin_propagates_output_between_agents():
    registry = DummyRegistry()
    backend = NativeOrchestrationBackend(registry=registry, message_bus: Optional[MessageBus] = None)
...
async def test_task_failure_marks_step_and_aggregates_failure_status():
    ...
    request = make_request(scenario="sequential", tasks=tasks)
