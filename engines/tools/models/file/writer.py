...
def write_file_tool(tool: Tool) -> dict[str, Any]:
    base = _base_dict(tool)
    if isinstance(tool, FileReadTool):
        base["file_path_template"] = tool.file_path_template
        base["encoding"] = tool.encoding
    elif isinstance(tool, FileWriteTool):
        base["file_path_template"] = tool.file_path_template
        base["content_template"] = tool.content_template
        base["encoding"] = tool.encoding
    return base
