...
def write_python_function_tool(tool: Tool) -> dict[str, Any]:
    base = _base_dict(tool)
    if isinstance(tool, PythonFunctionTool):
        base["module_path"] = tool.module_path
        base["function_name"] = tool.function_name
        base["import_type"] = tool.import_type
    return base
