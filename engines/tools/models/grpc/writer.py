...
def write_grpc_tool(tool: Tool) -> dict[str, Any]:
    base = _base_dict(tool)
    if isinstance(tool, GrpcServiceTool):
        base["host"] = tool.host
        base["port"] = tool.port
        base["service_name"] = tool.service_name
        base["method_name"] = tool.method_name
        base["proto_file_path"] = tool.proto_file_path
        base["tls_config"] = tool.tls_config
    return base
