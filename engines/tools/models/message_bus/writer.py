...
def write_message_bus_tool(tool: Tool) -> dict[str, Any]:
    base = _base_dict(tool)
    if isinstance(tool, MessageBusTool):
        base["transport"] = tool.transport
        base["topic"] = tool.topic
        base["message_template"] = tool.message_template
        base["publish"] = tool.publish
    return base
