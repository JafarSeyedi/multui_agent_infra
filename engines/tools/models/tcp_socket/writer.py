...
def write_tcp_socket_tool(tool: Tool) -> dict[str, Any]:
    base = _base_dict(tool)
    if isinstance(tool, TcpSocketTool):
        base["host"] = tool.host
        base["port"] = tool.port
        base["request_template"] = tool.request_template
        base["expect_response"] = tool.expect_response
        base["connection_timeout_ms"] = tool.connection_timeout_ms
    return base
