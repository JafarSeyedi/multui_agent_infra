...
def write_mib_snmp_tool(tool: Tool) -> dict[str, Any]:
    base = _base_dict(tool)
    if isinstance(tool, MibSnmpTool):
        base["host"] = tool.host
        base["port"] = tool.port
        base["community"] = tool.community
        base["snmp_version"] = tool.snmp_version.value
        base["oid"] = tool.oid
        base["operation"] = tool.operation
        base["value"] = tool.value
    return base
