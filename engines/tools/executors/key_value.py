...
@BaseToolExecutor.register(ToolKind.KEY_VALUE)
class KeyValueExecutor(BaseToolExecutor):
    ...
    async def _storage(self):
        ...
        try:
            if self._connection:
                self._backend_instance = create_storage("key_value", backend=self._backend, connection_string=self._connection)
            else:
                self._backend_instance = create_storage("key_value", backend=self._backend)
    ...
    @property
    def name(self) -> str:
        return f"kv:{self._backend}"

    @property
    def description(self) -> str:
        return f"Key-value storage via {self._backend} backend"
