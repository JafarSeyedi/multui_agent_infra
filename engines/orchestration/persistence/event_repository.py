...
class EventRepository(PersistentRuntimeRepository):
    ...
    def append(self, event: RawData) -> None:
        payload = dict(event)
    ...
    async def append_persisted(self, event: RawData) -> RawData:
        payload = dict(event)
        payload.setdefault("recorded_at", datetime.utcnow().isoformat())
    ...
    def by_correlation(self, correlation_id: str) -> list[RawData]:
    ...
    def by_correlation_osdm(self, correlation_key: RawData) -> list[RawData]:
        ...
        """
        def _matches_osdm_correlation(event: RawData) -> bool:
    ...
    def by_instance_and_time_ordered(self, instance_id: str) -> list[RawData]:
    ...
    def by_instance_and_time_ordered_desc(self, instance_id: str) -> list[RawData]:
    ...
    def by_time_range(self, start_time: str, end_time: str) -> list[RawData]:
