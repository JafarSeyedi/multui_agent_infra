...
from .master_writer import write_handout_master
from .master_writer import write_layout
from .master_writer import write_master
from .master_writer import write_notes_master
...
class PPTXWriter(BaseDocumentWriter):
    ...
    def _build_package(self, zf: zipfile.ZipFile, doc: PSDMDocument) -> None:
        ...
        # 5a. Handout Master
        if doc.handout_master:
            handout_xml = write_handout_master(doc.handout_master)
            zf.writestr("ppt/handoutMasters/handoutMaster1.xml", self._to_xml(handout_xml))

        # 5b. Notes Master
        if doc.notes_master:
            notes_master_xml = write_notes_master(doc.notes_master)
            zf.writestr("ppt/notesMasters/notesMaster1.xml", self._to_xml(notes_master_xml))
    ...
    def _build_presentation(self, doc: PSDMDocument) -> tuple[Element, list[tuple[str, str, str]]]:
        ...
        # Presentation attributes from parsed meta (stored in doc.metadata for pydantic compatibility)
        pres_meta = doc.metadata.get("presentation_attrs", {}) if hasattr(doc, 'metadata') else {}
        for attr, val in pres_meta.items():
            pres.set(attr, str(val))

        # Slide size
        SubElement(pres, f"{P}sldSz", {
        ...
        # Print settings
        prnDef = SubElement(pres, f"{P}prnDef")
        props = doc.presentation_properties
        if props.paper_size:
            prnDef.set("paperSize", props.paper_size)
        if props.paper_width:
            prnDef.set("paperWidth", str(props.paper_width))
        if props.paper_height:
            prnDef.set("paperHeight", str(props.paper_height))
        if props.scale:
            prnDef.set("scale", str(props.scale))
        if props.orientation:
            prnDef.set("orient", props.orientation)
        if props.paper_source:
            prnDef.set("paperSrc", props.paper_source)
        if props.first_slide_number:
            prnDef.set("firstSlideNum", str(props.first_slide_number))

        # Slide synchronization settings
        sldSync = SubElement(pres, f"{P}sldSync")
        sync_attrs: dict[str, Any] = {}
        sldSync.set("syncId", str(sync_attrs.get("sync_id", "")))
        if sync_attrs.get("scroll_outside") is not None:
            sldSync.set("scrollOutside", str(sync_attrs["scroll_outside"]).lower())
        if sync_attrs.get("zoom_outside") is not None:
            sldSync.set("zoomOutside", str(sync_attrs["zoom_outside"]).lower())

        # Custom shows
        if doc.custom_shows and doc.custom_shows.shows:
            custShowLst = SubElement(pres, f"{P}custShowLst")
            for idx, show in enumerate(doc.custom_shows.shows):
                custShow = SubElement(custShowLst, f"{P}custShow", {
                    "id": str(idx + 1),
                    "name": show.name
                })
                for slide_id in show.slide_ids:
                    SubElement(custShow, f"{P}sldId", {"id": str(slide_id)})
    ...
    def _build_content_types(self, doc: PSDMDocument) -> Element:
        ...
        overrides = [
            ...
            ("/ppt/handoutMasters/",    "application/vnd.openxmlformats-officedocument.presentationml.handoutMaster+xml"),
