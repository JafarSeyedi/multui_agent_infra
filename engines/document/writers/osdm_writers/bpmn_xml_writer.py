...
from ...models.osdm_models import (
    BaseOSDMDocument, BPMNDocument, BaseElement, RootElement, BpmnDefinitions, Import, Relationship,
    BPMNDiagram, BPMNPlane, BPMNShape, BPMNEdge, BPMNLabel, Bounds,
    Process, Collaboration, Choreography,
    Activity, Task, ServiceTask, SendTask, ReceiveTask, UserTask, ManualTask,
    ScriptTask, BusinessRuleTask, CallActivity, SubProcess, TransactionSubProcess,
    AdHocSubProcess, GlobalTask, GlobalUserTask, GlobalScriptTask, GlobalManualTask,
    GlobalBusinessRuleTask,
    FlowElement, FlowNode, SequenceFlow, MessageFlow,
    Gateway, ExclusiveGateway, InclusiveGateway, ParallelGateway, EventBasedGateway,
    ComplexGateway, Lane, LaneSet, Participant, PartnerEntity, PartnerRole,
    Event, StartEvent, EndEvent, IntermediateCatchEvent, IntermediateThrowEvent,
    BoundaryEvent, ImplicitThrowEvent,
    ...
    CompensateEventDefinition, ConditionalEventDefinition, LinkEventDefinition,
    CancelEventDefinition, TerminateEventDefinition,
    DataInput, DataOutput, InputSet, OutputSet, DataObject, DataStore,
    DataAssociation, DataInputAssociation, DataOutputAssociation,
    Property, ItemDefinition, Resource, ResourceRole, ResourceParameter,
    LoopCharacteristics, StandardLoopCharacteristics, MultiInstanceLoopCharacteristics,
    Message, Error, Escalation, Signal,
    CorrelationKey, CorrelationProperty, CorrelationSubscription,
    Category, CategoryValue, Artifact, Association, TextAnnotation, Group,
    Auditing, Monitoring, Rendering, FormalExpression, BpmnExpression,
    InputOutputSpecification, Operation, Interface, EndPoint,
    Conversation, ConversationNode, CallConversation, SubConversation,
    ConversationAssociation, ConversationLink, MessageFlowAssociation,
    ParticipantAssociation, ChoreographyActivity, ChoreographyTask,
    CallChoreography, SubChoreography, GlobalChoreographyTask,
)
...
class BPMNXMLWriter(BaseOSDMWriter):
    ...
    def __init__(self, options: Optional[OSDMWriteOptions] = None):
        ...
        # Mapping from object id → generated id (for deduplication if needed)
        self._id_map: Dict[str, str] = {}
        self._next_internal_id = 0

    # ── Public API (override to return bytes) ────────────────────
    async def _write_design(self, base_document: BaseOSDMDocument) -> bytes:
        root = Element(f"{{{BPMN_NS}}}definitions", {
            ...
            "xmlns:dc": DC_NS,
            "xmlns:xsi": XSI_NS,
            "id": self._get_or_create_id(document),
            "name": document.title or "definitions",
        })
        # Root elements: imports, relationships, etc.
        self._write_imports(root, base_document)
        self._write_relationships(root, base_document)
        document = cast(BPMNDocument, base_document)
        if document:
            if document.version:
                root.set("exporterVersion", document.version)
            # Top‑level elements: processes, collaborations, choreographies, global tasks
        ...
        self._write_root_elements(root, document)
        # Diagrams
    ...
    def _obj_id(self, obj: BaseElement) -> str:
        """Return the id to use in XML for a given model object."""
        return obj.id

    # ── Helpers to create elements ───────────────────────────────
    def _add_bpmn_element(self, parent: Element, tag: str, obj: Optional[BaseElement] = None, **attrs):
    ...
    # ── Write methods for each major component ───────────────────
    def _write_imports(self, root: Element, doc: BaseOSDMDocument):
    ...
    def _write_relationships(self, root: Element, doc: BaseOSDMDocument):
        for rel in doc.relationships:
            rel_elem = self._add_bpmn_element(root, "relationship", None, type=rel.type, direction=rel.direction.value)
    ...
    def _write_process(self, root: Element, process: Process):
        ...
        # Interfaces and operations (if any via supported interfaces)
        for iface in process.supported_interface_refs:
            # Already written in root elements? Not repeated here.
            pass
        # Input/Output specification if present at process level
        if process.io_specification:
            self._write_io_specification(proc_elem, process.io_specification)

    # ── Flow elements dispatch ───────────────────────────────────
    def _write_flow_element(self, parent: Element, flow: FlowElement):
        if isinstance(flow, Task):
            ...
            # Unknown flow element, skip (or could log warning)
            pass

    # ── Task subtypes ────────────────────────────────────────────
    def _write_task(self, parent, task: Task):
        elem = self._add_bpmn_element(parent, "task", task, name=task.name or "")
        self._write_activity_common(elem, task)

    def _write_service_task(self, parent, task: ServiceTask):
    ...
    def _write_send_task(self, parent, task: SendTask):
    ...
    def _write_receive_task(self, parent, task: ReceiveTask):
    ...
    def _write_user_task(self, parent, task: UserTask):
    ...
    def _write_manual_task(self, parent, task: ManualTask):
        elem = self._add_bpmn_element(parent, "manualTask", task, name=task.name or "")
        self._write_activity_common(elem, task)

    def _write_script_task(self, parent, task: ScriptTask):
    ...
    def _write_business_rule_task(self, parent, task: BusinessRuleTask):
        elem = self._add_bpmn_element(parent, "businessRuleTask", task, name=task.name or "")
        if task.implementation:
            # Implementation is a DecisionService – we could write its id as reference, but BPMN doesn't directly link.
            # We'll store the decision service id as a custom attribute? Standard BPMN uses implementation attribute.
            elem.set("implementation", "##unspecified")  # TODO: map to DecisionService
        self._write_activity_common(elem, task)

    # ── Common activity content ──────────────────────────────────
    def _write_activity_common(self, elem: Element, activity: Activity):
    ...
    def _write_sub_process(self, parent, sub: SubProcess):
    ...
    def _write_call_activity(self, parent, call: CallActivity):
        elem = self._add_bpmn_element(parent, "callActivity", call, name=call.name or "")
        if call.called_element:
            if isinstance(call.called_element, Process):
                elem.set("calledElement", self._obj_id(call.called_element))
            elif isinstance(call.called_element, GlobalTask):
                elem.set("calledElement", self._obj_id(call.called_element))
        self._write_activity_common(elem, call)

    # ── Gateways ─────────────────────────────────────────────────
    def _write_gateway(self, parent, gw: Gateway):
        tag = "gateway"  # we need to map gateway_type to tag name
        ...
        # Default sequence flow (if exclusive/inclusive/complex)
        if isinstance(gw, (ExclusiveGateway, InclusiveGateway, ComplexGateway)):
            if gw.default_sequence_flow:
        ...
        # Activation condition for complex gateway
    ...
    # ── Events ───────────────────────────────────────────────────
    def _write_event(self, parent, ev: Event):
        ...
        # Data associations
        if isinstance(ev, CatchEvent):
            for da in ev.data_output_associations:
                self._write_data_association(elem, da)
        if isinstance(ev, ThrowEvent):
            for da in ev.data_input_associations:
                self._write_data_association(elem, da)
        # Properties
        for prop in ev.properties:
            self._write_property(elem, prop)

    def _write_event_definition(self, parent: Element, ed: EventDefinition):
        tag_map = {
        ...
        if isinstance(ed, MessageEventDefinition):
            ...
            for src_id in ed.sources:
                SubElement(elem, f"{{{BPMN_NS}}}source", {"id": src_id})
            if ed.target:
                SubElement(elem, f"{{{BPMN_NS}}}target", {"id": ed.target})

    # ── Sequence Flow ────────────────────────────────────────────
    def _write_sequence_flow(self, parent, flow: SequenceFlow):
    ...
    # ── Data Objects / Stores ────────────────────────────────────
    def _write_data_object(self, parent, dobj: DataObject):
    ...
    def _write_data_object_reference(self, parent, ref):
    ...
    def _write_data_store(self, parent, store: DataStore):
    ...
    def _write_data_store_reference(self, parent, ref):
        elem = self._add_bpmn_element(parent, "dataStoreReference", ref)
        if ref.data_store:
            elem.set("dataStoreRef", self._obj_id(ref.data_store))

    # ── Data Associations ────────────────────────────────────────
    def _write_data_association(self, parent, da: DataAssociation):
        elem = self._add_bpmn_element(parent, "dataAssociation", da)
        for src in da.source_refs:
            SubElement(elem, f"{{{BPMN_NS}}}sourceRef", {"id": self._get_or_create_id(src)})
        if da.target_ref:
            SubElement(elem, f"{{{BPMN_NS}}}targetRef", {"id": self._get_or_create_id(da.target_ref)})
        if da.transformation:
            self._write_expression(elem, "transformation", da.transformation)

    def _write_data_input(self, parent, di: DataInput):
        elem = self._add_bpmn_element(parent, "dataInput", None, id=di.id, name=di.name)
        if di.item_subject_ref:
            elem.set("itemSubjectRef", self._obj_id(di.item_subject_ref))

    def _write_data_output(self, parent, do: DataOutput):
        elem = self._add_bpmn_element(parent, "dataOutput", None, id=do.id, name=do.name)
        if do.item_subject_ref:
            elem.set("itemSubjectRef", self._obj_id(do.item_subject_ref))

    # ── Input/Output Specification ───────────────────────────────
    def _write_io_specification(self, parent, iospec: InputOutputSpecification):
        ...
        for iset in iospec.input_sets:
            is_elem = self._add_bpmn_element(elem, "inputSet", iset, name=iset.name)
            for ref in iset.data_input_refs:
                SubElement(is_elem, f"{{{BPMN_NS}}}dataInputRefs", {"id": ref.data_input.id if ref.data_input else ""})
        for oset in iospec.output_sets:
            os_elem = self._add_bpmn_element(elem, "outputSet", oset, name=oset.name)
            for ref in oset.data_output_refs:
                SubElement(os_elem, f"{{{BPMN_NS}}}dataOutputRefs", {"id": ref.data_output.id if ref.data_output else ""})

    # ── Artifacts ────────────────────────────────────────────────
    def _write_artifact(self, parent, art: Artifact):
        if isinstance(art, Association):
            elem = self._add_bpmn_element(parent, "association", art, associationDirection=art.direction.value)
            if art.source_ref:
                elem.set("sourceRef", self._get_or_create_id(art.source_ref))
            if art.target_ref:
                elem.set("targetRef", self._get_or_create_id(art.target_ref))
    ...
    # ── Properties ───────────────────────────────────────────────
    def _write_property(self, parent, prop: Property):
        elem = self._add_bpmn_element(parent, "property", prop, name=prop.name)
        if prop.item_subject_ref:
            elem.set("itemSubjectRef", self._obj_id(prop.item_subject_ref))

    # ── Resources ────────────────────────────────────────────────
    def _write_resource_role(self, parent, role: ResourceRole):
    ...
    # ── Loop characteristics ─────────────────────────────────────
    def _write_loop_characteristics(self, parent, loop: LoopCharacteristics):
    ...
    def _write_complex_behavior_definition(self, parent, cbd):
    ...
    # ── Auditing / Monitoring ────────────────────────────────────
    def _write_auditing(self, parent, aud: Optional[Auditing]):
        if aud is None:
            return
        ...
        if aud.log_condition:
            elem.set("logCondition", aud.log_condition.body if aud.log_condition else "")

    def _write_monitoring(self, parent, mon: Optional[Monitoring]):
        if mon is None:
            return
        self._add_bpmn_element(parent, "monitoring", mon)

    # ── Collaboration / Choreography ─────────────────────────────
    def _write_collaboration(self, root, collab: Collaboration):
    ...
    def _write_choreography(self, root, choreo: Choreography):
        # Choreography extends Collaboration; we can write as choreography element
        elem = self._add_bpmn_element(root, "choreography", choreo, name=choreo.name or "")
        # Flow elements (choreography activities)
        ...
        # Collaboration parts (participants, message flows, etc.)
        # (duplicate code with collaboration – we could factor out)
        for participant in choreo.participants:
            self._write_participant(elem, participant)
        for msg_flow in choreo.message_flows:
            self._write_message_flow(elem, msg_flow)
        # ... other shared collaboration fields

    def _write_choreography_activity(self, parent, act: ChoreographyActivity):
    ...
    # ── Participants ─────────────────────────────────────────────
    def _write_participant(self, parent, part: Participant):
        ...
        if part.participant_multiplicity:
            ...
            mul_elem = SubElement(elem, f"{{{BPMN_NS}}}participantMultiplicity", {
                "minimum": str(mul.minimum),
                "maximum": str(mul.maximum) if mul.maximum > 0 else "*",
            })

    # ── Message Flow ─────────────────────────────────────────────
    def _write_message_flow(self, parent, mf: MessageFlow):
    ...
    # ── Global tasks ─────────────────────────────────────────────
    def _write_global_task(self, root, gt: GlobalTask):
        tag = "globalTask"
    ...
    # ── Root elements (Messages, Errors, etc.) ───────────────────
    def _write_root_elements(self, root, doc: BaseOSDMDocument):
        for re_obj in doc.root_elements.values():
    ...
    # ── Correlation ──────────────────────────────────────────────
    def _write_correlation_subscription(self, parent, sub: CorrelationSubscription):
    ...
    def _write_correlation_key(self, parent, key: CorrelationKey):
    ...
    def _write_conversation_node(self, parent, node: ConversationNode):
        tag = "conversation"
    ...
    def _write_conversation_association(self, parent, ca: ConversationAssociation):
    ...
    def _write_conversation_link(self, parent, link: ConversationLink):
    ...
    def _write_message_flow_association(self, parent, mfa: MessageFlowAssociation):
    ...
    def _write_participant_association(self, parent, pa: ParticipantAssociation):
    ...
    # ── Lane sets ────────────────────────────────────────────────
    def _write_lane_set(self, parent, lane_set: LaneSet):
    ...
    def _write_lane(self, parent, lane: Lane):
        elem = self._add_bpmn_element(parent, "lane", lane, name=lane.name or "")
        if lane.partition_element_ref:
            elem.set("partitionElement", self._get_or_create_id(lane.partition_element_ref))
    ...
    # ── Rendering (UserTask) ─────────────────────────────────────
    def _write_rendering(self, parent, rendering: Rendering):
        if isinstance(rendering, RenderingForm):
            elem = self._add_bpmn_element(parent, "rendering", rendering)
            # BPMN does not define a standard way to write rendering; we output as extension
    ...
    # ── Expressions ──────────────────────────────────────────────
    def _write_expression(self, parent, tag: str, expr: FormalExpression):
        elem = self._add_bpmn_element(parent, tag, expr)
        if expr.language:
            elem.set("language", expr.language.value if isinstance(expr.language, Enum) else str(expr.language))
        if expr.body:
            elem.text = expr.body

    # ── Diagrams ─────────────────────────────────────────────────
    def _write_diagram(self, root, diagram: BPMNDiagram):
    ...
    def _write_bpmn_shape(self, parent, shape: BPMNShape):
    ...
    def _write_bpmn_edge(self, parent, edge: BPMNEdge):
        elem = SubElement(parent, f"{{{BPMN_DI_NS}}}BPMNEdge", {
            "id": edge.id,
            "bpmnElement": self._obj_id(edge.model_element) if edge.model_element else "",
        })
        # Waypoints omitted for brevity; could be added later
        if edge.label:
            self._write_bpmn_label(elem, edge.label)

    def _write_bpmn_label(self, parent, label: BPMNLabel):
        SubElement(parent, f"{{{BPMN_DI_NS}}}BPMNLabel", {
            "id": label.id,
            "labelStyle": label.alignment.value if label.alignment else "left",
        })
    ...
    def _get_or_create_id(self, obj: Any) -> str:
        if isinstance(obj, BaseElement):
            return obj.id
        if isinstance(obj, (MSDMDocument, SSDM_DOCUMENT, TSDMDocument)):
            return obj.document_id or str(id(obj))
        if hasattr(obj, 'id'):
            return obj.id
        return str(id(obj))