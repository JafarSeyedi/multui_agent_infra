...
File size reduction and performance improvement
"""
import hashlib
import logging
import re
...
class OptimizationOptions:
    ...
    compress_images: bool = True          # Image compression
    compress_streams: bool = True         # Stream compression
...
class PDFOptimizer:
    ...
    def optimize(self, pdf_data: bytes) -> bytes:
        ...
        # Parse PDF into different parts
    ...
    def _optimize_images(self, pdf_parts: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            if 'objects' not in pdf_parts:
                logger.warning("PDF without objects for image optimization")
                return pdf_parts
            ...
            logger.error(f"Error in image optimization: {e}", exc_info=True)
            return pdf_parts


    def _is_image_object(self, obj: dict) -> bool:
        """
        Detect PDF image object
    ...
    def _optimize_single_image(self, image_obj: dict, options: dict) -> dict | None:
        ...
        try:
            ...
            logger.warning(f"Error in image optimization: {e}")
            return None
    ...
    def _optimize_png_image(self, image_data: bytes, image_info: dict, options: dict) -> bytes | None:
        ...
        try:
            ...
            logger.warning(f"Error in PNG optimization: {e}")
            return image_data
    ...
    def _optimize_tiff_image(self, image_data: bytes, image_info: dict, options: dict) -> bytes | None:
        ...
        try:
            ...
            logger.warning(f"Error in TIFF optimization: {e}")
            return image_data
    ...
    def _optimize_jbig2_image(self, image_data: bytes, image_info: dict, options: dict) -> bytes:
        ...
        (JBIG2 is already compressed, just check size)
        ...
        # Only check if downscaling is needed
        return image_data
    ...
    def _optimize_ccitt_image(self, image_data: bytes, image_info: dict, options: dict) -> bytes:
        ...
        Returns:
            Optimized data
        """
        # CCITT is optimized for black and white documents
        # Usually no further optimization needed
        return image_data
    ...
    def _compress_generic_image(self, image_data: bytes, image_info: dict, options: dict) -> bytes:
        ...
        try:
            ...
            # Only if compression is effective
    ...
    def _remove_unused_objects(self, pdf_parts: dict[str, Any]) -> dict[str, Any]:
        ...
        # This simple implementation only removes specified objects
    ...
    def _linearize_pdf(self, pdf_parts: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if 'objects' not in pdf_parts or not pdf_parts['objects']:
                logger.warning("PDF without objects for linearization")
                return pdf_parts
            ...
            # Order: Linearization Dict -> Root -> Pages -> Resources -> Content -> Others
            sorted_objects = []
            ...
            logger.error(f"Error in PDF linearization: {e}", exc_info=True)
    ...
    def _create_hint_tables(self, objects: list[dict], pages: list[dict]) -> dict[str, list]:
        ...
        Args:
            objects: Sorted object list
            pages: Page object list
        ...
        # Create Overflow Hint Table (for extra information)
        # Includes shared objects between pages
    ...
    def _create_hint_stream(self, hint_tables: dict[str, list]) -> bytes:
        ...
        hint_data.extend(struct.pack('>I', len(primary_table)))  # Page count
        ...
        hint_data.extend(struct.pack('>I', len(overflow_table)))  # Shared object count
    ...
    def _calculate_object_offset(self, obj: dict, all_objects: list[dict]) -> int:
        ...
        try:
            ...
            # Assumption: each object takes about 500 bytes
    ...
    def _calculate_object_length(self, obj: dict) -> int:
        ...
        Returns:
            Length in bytes
        """
        # Approximate length by object type
    ...
    def _identify_shared_objects(self, objects: list[dict], pages: list[dict]) -> list[dict]:
        """
        Identify objects shared between pages
        ...
        for obj in objects:
            if obj.get('type') in resource_types:
                # Check usage in more than one page
                used_in_pages = []
