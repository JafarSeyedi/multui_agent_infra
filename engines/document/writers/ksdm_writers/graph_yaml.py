...
class KSDMYAMLWriter(BaseDocumentWriter):
    ...
    async def write(self, document: KSDMDocument) -> bytes:
        data = {
            "version": "1.0",
            "entities": [{"id": e.id, "type": e.type.value if e.type else "Unknown"} for e in document.entities],
            "relations": [{"id": r.id, "source_id": r.source_id, "target_id": r.target_id} for r in document.relations],
        }
