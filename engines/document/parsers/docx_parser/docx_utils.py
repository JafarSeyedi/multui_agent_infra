...
class DocxUtils:
    ...
    def extract_paragraph_style(pPr_elem: ET.Element | None, styles_dict: dict[str, DocxStyleInfo]) -> dict[str, Any]:
        ...
        try:
            ...
            # Check keepLines (keep all lines on one page)
            ...
            # Check pageBreakBefore (break page before)
    ...
    def convert_omml_to_latex(omml_elem: ET.Element) -> str | None:
        ...
        """
        try:
            # This is a simple conversion. Full conversion requires a complete implementation
            latex_parts = []
            ...
            # If no specific conversion was done, extract as plain text
    ...
    def extract_hyperlink_info(hyperlink_elem: ET.Element) -> dict[str, Any]:
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).warning(f"Error extracting hyperlink info: {str(e)}")
    ...
    def extract_image_info(drawing_elem: ET.Element) -> dict[str, Any]:
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).warning(f"Error extracting image info: {str(e)}")
            ...
            if tblBorders_elem is not None:
                ...
                for border_type in ['top', 'left', 'bottom', 'right', 'insideH', 'insideV']:
                    ...
                    if border_elem is not None:
                        ...
                        if border_sz:
                            try:
                                # Convert from 1/8 point to point
    ...
    def extract_cell_properties(tcPr_elem: ET.Element | None) -> dict[str, Any]:
        ...
        try:
            ...
            if vMerge_elem is not None:
                ...
                if merge_val:
                    cell_props['v_merge'] = merge_val  # 'restart' or 'continue'
            ...
            import logging
            logging.getLogger(__name__).warning(f"Error extracting cell properties: {str(e)}")
    ...
    def extract_row_properties(trPr_elem: ET.Element | None) -> dict[str, Any]:
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).warning(f"Error extracting row properties: {str(e)}")
    ...
    def extract_footnote_info(footnote_elem: ET.Element) -> dict[str, Any]:
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).warning(f"Error extracting footnote info: {str(e)}")
    ...
    def extract_endnote_info(endnote_elem: ET.Element) -> dict[str, Any]:
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).warning(f"Error extracting endnote info: {str(e)}")
    ...
    def extract_bookmark_info(bookmark_elem: ET.Element) -> dict[str, Any]:
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).warning(f"Error extracting bookmark info: {str(e)}")
    ...
    def extract_comment_info(comment_elem: ET.Element) -> dict[str, Any]:
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).warning(f"Error extracting comment info: {str(e)}")
    ...
    def extract_field_info(fldChar_elem: ET.Element) -> dict[str, Any]:
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).warning(f"Error extracting field info: {str(e)}")
    ...
    def extract_smart_tag_info(smartTag_elem: ET.Element) -> dict[str, Any]:
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).warning(f"Error extracting Smart Tag info: {str(e)}")
    ...
    def extract_sdt_info(sdt_elem: ET.Element) -> dict[str, Any]:
        """
        Extract Structured Document Tag (form controls) information
        ...
        Returns:
            Dict[str, Any]: SDT information including:
                - type: SDT type (text, richText, comboBox, dropDownList, date, etc.)
                - tag: Tag name
                - alias: Display name
                - lock: Lock status
                - placeholder: Placeholder text
                - content: Current content
                - list_items: List items (for comboBox and dropDownList)
                - date_format: Date format (for date picker)
                - formatting: Additional formatting
        """
        sdt_info = {
            'type': 'unknown',
            'tag': None,
        ...
        try:
            ...
            if sdtPr_elem is not None:
                ...
                # Extract additional formatting for text and richText
            ...
            import logging
            logging.getLogger(__name__).warning(f"Error extracting SDT info: {str(e)}")
    ...
    def extract_math_info(math_elem: ET.Element) -> dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Math information including:
                - type: Type (inline, paragraph)
                - latex: LaTeX representation (if conversion successful)
                - omml_xml: Original OMML XML
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).warning(f"Error extracting math info: {str(e)}")
    ...
    def extract_drawing_info(drawing_elem: ET.Element) -> dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Drawing information including:
                - type: Type (picture, shape, chart, diagram, etc.)
                - id: ID
                - name: Name
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).warning(f"Error extracting Drawing info: {str(e)}")
    ...
    def extract_header_footer_info(part_xml: ET.Element) -> dict[str, Any]:
        ...
        Args:
            part_xml: Root XML element of header or footer
            
        Returns:
            Dict[str, Any]: Header/footer information including:
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).warning(f"Error extracting header/footer info: {str(e)}")
    ...
    def extract_section_properties(sectPr_elem: ET.Element) -> dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Section information including:
                ...
                - page_margins: Margins (top, right, bottom, left, header, footer, gutter)
                - page_orientation: Page orientation (portrait, landscape)
                - page_numbering: Page numbering
                - columns: Columns (count, spacing)
                - header_references: References to headers
                - footer_references: References to footers
    ...
    def extract_page_break_info(br_elem: ET.Element) -> dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Page break information including:
                - type: Type (page, column, textWrapping)
                - clear: Clear type (for textWrapping)
        ...
        try:
            ...
            # Determine location (based on parent element)
            parent = br_elem.getparent()
            if parent is not None:
                # If break is at the start of a paragraph
            ...
            import logging
            logging.getLogger(__name__).debug(f"Error extracting page break info: {str(e)}")
    ...
    def extract_column_break_info(br_elem: ET.Element) -> dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Column break information including:
                - type: Always 'column'
                - location: Location (before, after)
        """
        column_break_info = {
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).debug(f"Error extracting column break info: {str(e)}")
    ...
    def extract_line_break_info(br_elem: ET.Element) -> dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Line break information including:
                - type: Always 'textWrapping'
                - clear: Clear type (all, left, right, none)
                - location: Location (before, after)
        """
        line_break_info = {
            'type': 'textWrapping',
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).debug(f"Error extracting line break info: {str(e)}")
    ...
    def extract_tab_info(tab_elem: ET.Element) -> dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Tab information including:
                - type: Tab type (left, center, right, decimal, bar, clear, leader)
        ...
        try:
            ...
            if parent is not None and parent.tag.endswith('tabs'):
                ...
                for tab_stop in parent.findall('.//w:tab', OOXML_NAMESPACES):
                    # Check based on position (if available)
            ...
            import logging
            logging.getLogger(__name__).debug(f"Error extracting tab info: {str(e)}")
    ...
    def extract_sym_info(sym_elem: ET.Element) -> dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Symbol information including:
                - font: Symbol font
                - char: Symbol character (unicode code)
        ...
        try:
            ...
            if char:
                ...
                # Convert to unicode
                try:
                    # Character is in hexadecimal format
            ...
            import logging
            logging.getLogger(__name__).debug(f"Error extracting symbol info: {str(e)}")
    ...
    def extract_fld_char_info(fldChar_elem: ET.Element) -> dict[str, Any]:
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).debug(f"Error extracting field char info: {str(e)}")
    ...
    def extract_soft_hyphen_info(hyphen_elem: ET.Element) -> dict[str, Any]:
        ...
        Args:
            hyphen_elem: w:softHyphen or w:noBreakHyphen element
            
        Returns:
            Dict[str, Any]: Hyphen information including:
                - type: Type (softHyphen, noBreakHyphen)
                - char: Hyphen character
        """
        hyphen_info = {
            'type': 'softHyphen',
            'char': '­'  # Soft hyphen character (U+00AD)
        }
        ...
        try:
            ...
            if hyphen_elem.tag.endswith('noBreakHyphen'):
                ...
                hyphen_info['char'] = '‑'  # Non-breaking hyphen character (U+2011)

        except Exception as e:
            import logging
            logging.getLogger(__name__).debug(f"Error extracting hyphen info: {str(e)}")
    ...
    def extract_year_short_info(year_short_elem: ET.Element) -> dict[str, Any]:
        ...
        try:
            ...
            # But it may have format properties
            ...
            import logging
            logging.getLogger(__name__).debug(f"Error extracting year short info: {str(e)}")
    ...
    def extract_month_short_info(month_short_elem: ET.Element) -> dict[str, Any]:
        ...
        try:
                        ...
                        if fld_char_info.get('instrText') and 'DATE' in fld_char_info['instrText']:
                            ...
                            if '\\@' in instr_text:
                                ...
                                if 'MMMM' in format_part:
                                    month_short_info['format'] = 'MMMM'  # Full month name
                                elif 'MMM' in format_part:
                                    month_short_info['format'] = 'MMM'  # Short month name
                                elif 'MM' in format_part:
                                    month_short_info['format'] = 'MM'  # Two-digit month
                                elif 'M' in format_part:
                                    month_short_info['format'] = 'M'  # One or two-digit month
                            break
            ...
            import logging
            logging.getLogger(__name__).debug(f"Error extracting month short info: {str(e)}")
    ...
    def extract_day_short_info(day_short_elem: ET.Element) -> dict[str, Any]:
        ...
        try:
                        ...
                        if fld_char_info.get('instrText') and 'DATE' in fld_char_info['instrText']:
                            ...
                            if '\\@' in instr_text:
                                ...
                                if 'dddd' in format_part:
                                    day_short_info['format'] = 'dddd'  # Full day name
                                elif 'ddd' in format_part:
                                    day_short_info['format'] = 'ddd'  # Short day name
                                elif 'dd' in format_part:
                                    day_short_info['format'] = 'dd'  # Two-digit day
                                elif 'd' in format_part:
                                    day_short_info['format'] = 'd'  # One or two-digit day
                            break
            ...
            import logging
            logging.getLogger(__name__).debug(f"Error extracting day short info: {str(e)}")
