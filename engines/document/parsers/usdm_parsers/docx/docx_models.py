...
class DOCXField:
    ...
    # Cross-reference target bookmark
    target_bookmark: str | None = None

    # TOC switches
    toc_switches: dict[str, Any] | None = None
    ...
    form_field_type: str | None = None
    form_field_name: str | None = None
    form_field_default: str | None = None
...
class DOCXNumberingLevel:
    ...
    # Legal numbering (use parentheses for all)
    is_legal: bool = False

    # Restart numbering condition
    restart_condition: str | None = None

    # Paragraph formatting override
    paragraph_props: dict[str, Any] | None = None

    # Run formatting override
    run_props: dict[str, Any] | None = None
...
class DOCXHeaderFooter:
    ...
    # Watermarks found in this header/footer
    watermarks: list[DOCXWatermark] = field(default_factory=list)
...
class DOCXSection:
    ...
    # Header/footer relationship mappings
    ...
    page_borders: dict[str, Any] | None = None
...
@dataclass
class DOCXTOCField:
    """Intermediate model for TOC field data."""
    instruction: str = ""
    switches: dict[str, Any] = field(default_factory=dict)
    heading_range: tuple | None = None
    styles_included: list[tuple[str, int]] = field(default_factory=list)
    hyperlinks: bool = False
    hide_web_layout: bool = False
    use_paragraph_levels: bool = False
...
@dataclass
class DOCXWatermark:
    """Intermediate model for watermark data."""
    watermark_id: str = ""
    text: str | None = None
...
# ============================================================
# RTL / BIDIRECTIONAL TEXT DATA
# ============================================================

@dataclass
class DOCXRTLProperties:
    """RTL/bidirectional properties at various levels."""
    paragraph_rtl: bool = False
    run_rtl: bool = False
    cell_direction: str | None = None
    table_rtl: bool = False
    section_rtl: bool = False
    text_direction_raw: str | None = None
...
@dataclass
class DOCXChartData:
    """Intermediate model for chart data parsed from chart XML parts."""
    chart_id: str = ""
    ...
    category_axis: dict | None = None
    value_axis: dict | None = None
...
@dataclass
class DOCXComplexField:
    """Intermediate model for complex (multi-run) field data."""
    field_type: str = ""
    instruction: str = ""
...
# ============================================================
# NUMBERING OVERRIDE DATA
# ============================================================

@dataclass
class DOCXNumberingOverride:
    """Complete numbering override for a level."""
    level: int = 0
...
class DOCXDocument:
    ...
    # TOC fields
    toc_fields: list[DOCXTOCField] = field(default_factory=list)

    # Watermarks collected from headers
    watermarks: list[DOCXWatermark] = field(default_factory=list)

    # Complex fields (multi-run fields)
    complex_fields: list[DOCXComplexField] = field(default_factory=list)

    # Chart data (by relationship ID)
    chart_data: dict[str, DOCXChartData] = field(default_factory=dict)

    # Numbering overrides
    numbering_overrides: dict[str, list[DOCXNumberingOverride]] = field(default_factory=dict)

    # RTL properties at document level
    rtl_properties: DOCXRTLProperties = field(default_factory=DOCXRTLProperties)
