...
class DOCXParser:
    ...
    def _apply_rtl_to_paragraph(self, para: DOCXParagraph, elem: LogicalElement) -> LogicalElement:
        """Apply RTL properties to a converted logical element."""
    ...
    def _apply_rtl_to_run(self, run: DOCXTextRun) -> dict[str, str]:
        """Apply RTL properties to a text run's metadata."""
    ...
    def _apply_rtl_to_table(self, table: DOCXTable) -> dict[str, Any]:
        """Apply RTL properties to a table's metadata."""
        metadata: dict[str, Any] = {}
        if table.properties:
    ...
    def _get_text_direction_value(self, direction_str: str | None) -> str:
        """Map OOXML text direction values to USDM direction."""
    ...
    # ============================================================
    # CHART XML PARTS CONVERSION
    # ============================================================

    def _convert_chart_xml_parts(self) -> list[LogicalElement]:
        """Convert all chart XML parts to ChartContent logical elements."""
    ...
    def _convert_single_chart_data(self, rel_id: str, chart_data: DOCXChartData) -> LogicalElement | None:
        """Convert DOCXChartData to ChartContent logical element."""
        type_map = {
        ...
        series_list: list[ChartSeriesContent] = []
        for s in chart_data.series:
            series_list.append(ChartSeriesContent(
                name=s.get('name'),
                categories_ref=s.get('categories_ref'),
                values_ref=s.get('values_ref'),
        ...
        cat_axis = None
        val_axis = None
        if chart_data.category_axis:
            cat_axis = ChartAxisContent(
                axis_type='category',
                title=None,
                min_value=chart_data.category_axis.get('min'),
                max_value=chart_data.category_axis.get('max'),
                format_code=chart_data.category_axis.get('format_code'),
            )
        if chart_data.value_axis:
            val_axis = ChartAxisContent(
                axis_type='value',
                title=None,
                min_value=chart_data.value_axis.get('min'),
                max_value=chart_data.value_axis.get('max'),
                format_code=chart_data.value_axis.get('format_code'),
            )

        content = ChartContent(
            chart_type=chart_type,
            grouping=chart_data.grouping,
    ...
    # ============================================================
    # MULTI-LEVEL NUMBERED LIST OVERRIDES
    # ============================================================

    def _convert_list_styles_with_overrides(self) -> dict[str, ListStyle]:
        """Convert DOCX numbering definitions to USDM ListStyle with override support."""
        list_styles: dict[str, ListStyle] = {}
        assert self._docx_doc is not None, "Document not extracted"
        ...
        for num_id, instance in self._docx_doc.numbering_instances.items():
            if instance.levels_overrides:
                ...
                if override_levels:
                    override_style = ListStyle(
                        name=override_style_name,
                        level_styles=override_levels
                    )
                    list_styles[override_style_name] = override_style

        return list_styles

    def _convert_complex_field(self, field: DOCXField) -> DataContent | TOCContent | LinkContent | PageReferenceContent | CrossReference | FormFieldContent | None:
        """Convert a complex field (from DOCXComplexField intermediate)."""
