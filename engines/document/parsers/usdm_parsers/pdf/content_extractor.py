...
Extract text, tables, images, links, and other content from PDF files
"""
import base64
import io
import json
...
class ContentExtractor:
    """
    Main class for extracting PDF content
    """
    ...
    def extract_all(self,
        ...
        Args:
            layout: LayoutAnalyzer output (optional)
            structure: StructureParser output (optional)
        ...
        try:
            ...
            print(f"✅ Extraction complete! Time: {self.stats.extraction_time:.2f} seconds")
    ...
    def _extract_text_direct(self,
        ...
        with pdfplumber.open(self.pdf_path) as pdf:
            for page_num, page in enumerate(pdf.pages, 1):
                try:
                    # Extract text با pdfplumber
                    text = page.extract_text()

                    if text and text.strip():
                        # Extract text preserving position
                        chars = page.chars
                        ...
                        if chars:
                            ...
                            # If characters are not available, save the full page text
                            extracted_text = ExtractedText(
                                text=text,
                                page_num=page_num,
                                bbox=page.bbox,
                ...
                if base_image:
                    ...
                    if width >= min_size[0] and height >= min_size[1]:
                        ...
                        extracted_image = ExtractedImage(
                            page_num=page_num + 1,
                            bbox=(0, 0, width, height),  # Exact position needs further processing
    ...
    def _extract_links_pdfplumber(self):
        ...
        with pdfplumber.open(self.pdf_path) as pdf:
            for page_num, page in enumerate(pdf.pages, 1):
                ...
                # Links can be found from extracted text
                text = page.extract_text()
                if text:
...
def extract_content_from_pdf(pdf_path: str,
    ...
    """
    Helper function for extracting PDF content
    
    Args:
        pdf_path: PDF file path
        use_ocr: use OCR
        output_json: JSON save path (optional)
        output_dir: directory for CSV and images (optional)
...
#     print(f"✅ Extraction complete! Results saved in extraction_results.json.")
