...
from engines.document.models.isdm_models import MiningModelType, MlMiningDocument

ONNX_AVAILABLE = importlib.util.find_spec('onnx') is not None


class OnnxParser(BaseKnowledgeParser):
    ...
    def parse(self, source: str | Path | BinaryIO, **options: Any) -> ParseResult:
        if not ONNX_AVAILABLE:
            raise KnowledgeParseError("ONNX parsing requires 'onnx' package. Install with: pip install onnx")
        try:
            import onnx
            ...
            raise KnowledgeParseError(f"ONNX parse failed: {e}")
