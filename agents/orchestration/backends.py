...
class NativeOrchestrationBackend:
    ...
    async def execute(self, request: OrchestrationRequest) -> OrchestrationResult:
        ...
        if request.scenario == "sequential":
            await self._execute_sequential(request, shared_context, steps, executions, messages)
        elif request.scenario == "broadcast":
            await self._execute_broadcast(request, shared_context, steps, executions, messages)
        elif request.scenario == "round_robin":
            await self._execute_round_robin(request, shared_context, steps, executions, messages)
        elif request.scenario == "selector":
            await self._execute_selector(request, shared_context, steps, executions, messages, notes)
    ...
    async def _execute_sequential(self, request, shared_context, steps, executions, messages):
    ...
    async def _execute_broadcast(self, request, shared_context, steps, executions, messages):
    ...
    async def _execute_round_robin(self, request, shared_context, steps, executions, messages):
    ...
    async def _execute_selector(self, request, shared_context, steps, executions, messages, notes):
...
class AutoGenOrchestrationBackend:
    ...
    async def execute(self, request: OrchestrationRequest) -> OrchestrationResult:
        if not self.is_available():
            result = await self.native_backend.execute(request)
